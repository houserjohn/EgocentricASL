...
def update_plot(frame, all_frame_points, scat, lines, hand_lines_left, hand_lines_right):
    ...
    if pose_points is not None:
        ...
        for i in [32, 30, 28, 31, 29, 27, 26, 24, 25, 23, 18, 20, 22, 21, 19, 17, 11, 12, 10, 9, 7, 3, 2, 1, 0, 4, 5, 6, 8]:
            pose_points[i, :] = (-inf, -inf, -inf)
    ...
    if pose_points is not None:
        offsets2d = pose_points

    if left_hand_points is not None:
        offsets2d = np.concatenate([offsets2d, left_hand_points])  

    if right_hand_points is not None:
        offsets2d = np.concatenate([offsets2d, right_hand_points])

    offsets2d = rotate_y(offsets2d, 180)
    ...
    if pose_points is not None:
        for line, conn in zip(lines, connections):
            ...
            line.set_data(np.array([tmp_points[conn[0], 0], tmp_points[conn[1], 0]]),
                            np.array([tmp_points[conn[0], 1], tmp_points[conn[1], 1]]))
...
def main(filepath, output_dir, preview):
    ...
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.invert_yaxis()  # Invert the Y-axis so negative values are upwards
